  ...
  function checkDriveKeys () {
    ...
    drive.core.list(function (err, vals) {
      ...
      if (vals.length > 2 && !opts.key) return cb(new Error('Drive has multiple archives. Must specify key.'))
        ...
        if (vals.length && vals.indexOf(opts.key) === -1) return cb(new Error('Existing archive in database does not match key option.'))
        archive = drive.createArchive(opts.key, opts)
        archive.resumed = false
        return doneArchive(!vals.length && opts.key) // do not open if new drive and external key (e.g. downloading)
      }
