...
class DatStats extends EventEmitter {
  ...
  get peers () {
    ...
    var peers = this.archive.content.peers
  ...
  update () {
    ...
    var current = this.archive.tree.checkout(this.archive.version)
    var initial = this.archive.tree.checkout(this._count.version)
    ...
    function wait () {
      self.archive.metadata.update(() => {
        ...
        console.log('jlkadjfal;skdjfal;skjdf')
        self.update()
      })
    }
  }
  ...
  _runDownloadStats () {
    if (!this.archive && !this.archive.content) return this.archive.once('content', this._runDownloadStats)

    var feed = this.archive.content
    this._count.downloaded = 0
    ...
    this.archive.content.on('download', countDown)
    this.archive.content.on('clear', checkDownloaded)
